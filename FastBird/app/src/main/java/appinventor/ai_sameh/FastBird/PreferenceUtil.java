...
public class PreferenceUtil {
	...
	public static ArrayList<MRBTransactions> getCashHistory(Context context) {
		String money = getPref(context).getString(CASH_HISTORY, "[]");
		Type listType = new TypeToken<ArrayList<MRBTransactions>>() {
		}.getType();

		ArrayList<MRBTransactions> mrbTransactionsArrayList = new Gson().fromJson(money, listType);
		Collections.sort(mrbTransactionsArrayList, new Comparator<MRBTransactions>() {
			@Override
			public int compare(MRBTransactions lhs, MRBTransactions rhs) {
				try {
					return TimestampUtil.getIso8601Date(rhs.getDate()).compareTo(TimestampUtil.getIso8601Date(lhs.getDate()));
				} catch (ParseException e) {
					return -1;
				}
			}
		});
		return mrbTransactionsArrayList;
	}
